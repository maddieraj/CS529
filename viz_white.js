...
Promise.all([
    ...
    d3.json('json/statedata.json'),
    d3.json('json/citydata.json'),
    d3.json('json/sgd.json')
])
...
function loadStateData(d) {
    ...
    let code = stateData[d.properties.NAME].state_code;

    DTABLE.destroy()
    $('#maintable').empty();

    let filtered_citydata = cityData.filter(x => x.state_code == code);
    let arr_fAges = [];
